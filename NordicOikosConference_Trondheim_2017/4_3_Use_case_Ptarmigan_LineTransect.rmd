...
To make the data ready for analysis and exploration, we need to do some data wrangling. First, we add sampling year (eventYear) to the data table using the `year` function in library [lubridate](https://cran.r-project.org/web/packages/lubridate/index.html). We also use the function `colsplit` in library *reshape2* to split the line string in the column dynamicProperties. 
